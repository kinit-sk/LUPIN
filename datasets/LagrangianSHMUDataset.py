...
class LagrangianSHMUDataset(Dataset):
    """A Dataset for working with SHMU radar files that are in h5 format and transformed to Lagrangian coordinates."""
    ...
    def __init__(
        ...
        with h5py.File(date_list.format(split=split), 'r') as hf:
            ...
            self.date_list = np.array([datetime.strptime(item, '%Y-%m-%dT%H:%M:%S.%fZ') for item in self.date_list])

        self.path = path
        self.filename = filename

        # Get correct importer function
        if importer == "lagrangian_h5":
        ...
        self.date_list_pdt = self.date_list
    ...
    def make_windows(self):
        # Get windows
        windows = np.lib.stride_tricks.sliding_window_view(self.date_list, self.num_frames)
        filtered_windows = []
        for window in windows:
            if (window[-1] - window[0]).seconds / (self.timestep * 60) == (self.num_frames - 1):
