"""This script will run nowcasting prediction for the LUPIN model implementation
"""
import argparse
...
def run(checkpointpath, configpath, predict_list) -> None:
    ...
    model = LUPIN(modelconf).load_from_checkpoint(checkpointpath, config=modelconf, map_location=torch.device('cpu'))
